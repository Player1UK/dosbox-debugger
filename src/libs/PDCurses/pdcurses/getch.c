...
static void _highlight(void)
{
    ...
    int colStart = ((x_start / colWidth) * colWidth);
    int colEnd = colStart + colWidth;
    for (j = y_start; j <= y_end && j < curscr->_maxy; j++)
        for (i = (j == y_start ? x_start : colStart);
             i < (j == y_end ? x_end : colEnd); i++)
...
# define MASK 0xff
    ...
    int colStart = ((x_start / colWidth) * colWidth);
    int colEnd = colStart + colWidth;
    for (j = y_start, pos = 0; j <= y_end && j < curscr->_maxy; j++)
    {
        for (i = (j == y_start ? x_start : colStart);
             i < (j == y_end ? x_end : colEnd); i++)
